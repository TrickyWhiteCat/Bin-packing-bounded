...
import time
import os
from src.solver.hill_climbing import *
from src.solver.simulated_annealing import *
...
def test(N, K, time_limit:int = 60, data_path:str = None, result_file:str = None, create_new_data:bool = True):
    ...
    solvers= [GreedySolver(input_file=data_path),Hillclimbing(input_file=data_path), Simulatedannealing(input_file=data_path),CPSolver(input_file=data_path, use_greedy=False, time_limit=time_limit, log_cp_sat_process = False),
               ILPSolver(input_file=data_path, time_limit=time_limit),
               CPSolver(input_file=data_path, use_greedy=True, time_limit=time_limit, log_cp_sat_process = False)]
