...
class GreedySolver:
    ...
    def __add_goods(self, prioritize: str = "quantity"):
        ...
        #bo qua check lower them, vi cai nay sat lower roi, nen chi can thay doi xiu la dc 
    ...
    def solve(self):
        ...
        for key in ["efficiency", "value", "quantity"]:
            ...
            temp= self.__add_goods(prioritize=key)
            value = self.objective_function(temp)
            if max_value <= value:
                max_value = value
                max_value_solution = temp
            
        goods_pos = max_value_solution
